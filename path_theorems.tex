...
\end{theorem}
\proof Let $p_1$ be a particle whose path $\gamma_{p_1}$ is determined by the minimizing $|\theta(t)|$ for all times $t \in [T_0(\bhat{\gamma_p}), T_f(\bhat{\gamma_p})]$. We shall show that $\gamma_{p_1}$ is in fact the fastest path to $\bvec{X}_f$.
